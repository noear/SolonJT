package org.noear.solonjt.actuator.python;
...
public class PythonJtActuator implements IJtActuator {
    private static final ThData<StringBuilder> _tlBuilder = new ThData(new StringBuilder(1024 * 5));
    private static final String _lock = "";
    private static PythonJtActuator _g;

    public static PythonJtActuator singleton() {
        if (_g == null) {
            synchronized (_lock) {
                if (_g == null) {
                    _g = new PythonJtActuator();
                }
            }
        }
    ...
    public static class __JTEAPI {
        public String require(String path) throws Exception {
            ...
            PythonJtActuator.singleton().preLoad(name2, file);
