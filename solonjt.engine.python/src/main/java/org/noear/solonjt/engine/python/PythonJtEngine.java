...
public class PythonJtEngine implements IJtEngine {
    private static final ThData<StringBuilder> _tlBuilder = new ThData(new StringBuilder(1024 * 5));
    private static final String _lock = "";
    private static PythonJtEngine _g;

    public static PythonJtEngine singleton() {
        if (_g == null) {
            synchronized (_lock) {
                if (_g == null) {
                    _g = new PythonJtEngine();
                }
            }
        }
    ...
    private PythonJtEngine() {
        ...
        try {
            ...
            sb.append("def modelAndView(tml,mod):\n" +
                    "    return __JTEAPI.modelAndView(tml,mod);").append("\n");

            sb.append("def require(path):\n" +
                    "    __JTEAPI.require(path);" +
                    "    return __global['lib'][path];").append("\n");
    ...
    public static class __JTEAPI {
        public String require(String path) throws Exception {
            ...
            PythonJtEngine.singleton().preLoad(name2, file);
