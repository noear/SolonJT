 ...
 * javascript 代码运行工具
 * */
public class JavascriptJtActuator implements IJtActuator {
    private static final ThData<StringBuilder> _tlBuilder = new ThData(new StringBuilder(1024*5));
    private static final String _lock ="";
    private static JavascriptJtActuator _g;
    public static JavascriptJtActuator singleton(){
        if(_g == null){
            synchronized (_lock){
                if(_g == null){
                    _g = new JavascriptJtActuator();
                }
            }
        }
